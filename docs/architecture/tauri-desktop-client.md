...
主窗口使用操作系统原生外框，由 Windows 直接提供拖动、最小化、最大化和关闭按钮。关闭请求仍由 Tauri 拦截，以实现按设置驻留托盘；窗口基础操作不依赖远程 Web 页面的 IPC 命令。
...
桌面端不能仅凭域名可解析或 TCP 端口可连接判断 Kalender 可用。Cloudflare、反向代理和网关即使返回错误页，端口也仍然可连接。客户端统一请求已配置服务器根路径下的 `/api/health`，并只在 HTTP 成功且响应同时包含 `ok: true` 与 `status: "healthy"` 时判定服务正常。

- 启动时健康检查失败，只启动托盘和本地提醒调度器，不创建主窗口；
- 托盘打开、通知跳转和设置跳转前重新检查，失败时打开服务器配置窗口而不是远程错误页；
- 保存服务器地址后立即检查，失败时保留配置窗口并每 30 秒后台重试；
- 服务运行中断时隐藏主窗口，避免窗口停留在代理错误页；
- 服务恢复后静默创建或刷新隐藏窗口以恢复同步，不主动抢占焦点；
- 托盘菜单和悬停提示显示健康检查状态，缓存中的提醒在离线期间继续调度。
