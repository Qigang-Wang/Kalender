...
export function parseEntityReference(kind: unknown, entityId: unknown): { readonly kind: EntityKind; readonly entityId: string } {
  if (!entityKinds.includes(kind as EntityKind)) throw new EntityLinkValidationError("Objekttyp ungültig");
  if (typeof entityId !== "string" || !entityId.trim() || entityId.length > 500) throw new EntityLinkValidationError("Objekt-ID ungültig");
...
export function parseEntityLinkInput(body: EntityLinkRequestBody | null): SaveEntityLinkInput {
  if (!body) throw new EntityLinkValidationError("fehlende Verbindungsinformationen");
  ...
  if (!/^[a-z][a-z0-9-]{0,49}$/.test(relation)) throw new EntityLinkValidationError("Ungültiger Assoziationstyp");
