...
async function verifyLegacyUpgrade(database: TestPostgresDatabase) {
  ...
  const backupSettings = await database.query<{ retention_count: number; encrypt_automatic: boolean }>(
    "SELECT retention_count, encrypt_automatic FROM backup_settings WHERE id = 'workspace'",
  );
  assert(backupSettings.rows[0]?.retention_count === 3, "legacy automatic backup retention is reduced to three");
  assert(backupSettings.rows[0]?.encrypt_automatic === false, "untouched automatic backup settings do not require encryption");
