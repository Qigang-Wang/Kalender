...
import { deleteCalendarEvent, upsertCalendarEvent } from "./calendar-event-service";
...
import { deleteEntityLink, listRelatedEntities, saveEntityLink, type EntityKind } from "./entity-link-repository";
...
import { deleteStoredNote, getStoredNote, listStoredNotes, listStoredProjectNotes, listStoredProjects, saveStoredNote, type SaveNoteInput, type StoredNote, type StoredProject } from "./note-repository";
import { deleteStoredProjectPlanItem, listStoredProjectPlanItems, saveStoredProjectPlanItem, type SaveProjectPlanItemInput, type StoredProjectPlanItem } from "./project-plan-repository";
import { getStoredProjectOverview } from "./project-repository";
import { deleteStoredTaskSchedule, parseTaskScheduleInput, scheduleStoredTask, updateStoredTaskSchedule, type ScheduledTaskResult } from "./task-schedule";
import { getStoredTask, listStoredProjectTasks, listStoredTasks, saveStoredTask, TaskRepositoryError, type StoredTask } from "./task-repository";
...
export interface McpDomainOperationMap {
  ...
  readonly dayline_tasks_list: { input: { includeCompleted?: boolean; projectId?: string }; output: readonly StoredTask[] };
  ...
  readonly dayline_projects_list: { input: { includeArchived?: boolean }; output: readonly StoredProject[] };
  ...
  readonly dayline_project_plan_items_list: { input: { projectId: string }; output: readonly StoredProjectPlanItem[] };
  ...
  readonly dayline_relations_list: { input: { kind: EntityKind; entityId: string }; output: unknown };
...
export class McpDomainService {
  ...
  private async executeScoped(name: McpDomainOperationName, raw: Record<string, unknown>): Promise<unknown> {
    switch (name) {
      ...
      case "dayline_tasks_list": return raw.projectId ? listStoredProjectTasks(requiredText(raw.projectId, "项目标识"), raw.includeCompleted === true) : listStoredTasks(raw.includeCompleted === true);
      case "dayline_task_get": return getStoredTask(requiredText(raw.taskId, "任务标识"));
      case "dayline_projects_list": return listStoredProjects(raw.includeArchived === true);
      ...
      case "dayline_project_plan_items_list": return listStoredProjectPlanItems(requiredText(raw.projectId, "项目标识"));
      ...
      case "dayline_relations_list": return listRelatedEntities(entityKind(raw.kind), requiredText(raw.entityId, "对象标识"));
      case "dayline_task_create": return this.write(name, raw, () => saveStoredTask(parseTaskInput(raw as TaskRequestBody)), () => preview(null, raw));
      ...
      case "dayline_relation_link": return this.write(name, raw, () => saveEntityLink({ sourceKind: entityKind(raw.sourceKind), sourceId: requiredText(raw.sourceId, "来源标识"), targetKind: entityKind(raw.targetKind), targetId: requiredText(raw.targetId, "目标标识"), relation: optionalText(raw.relation) ?? "related" }), () => preview(null, raw));
  ...
  private async taskUpdate(operation: string, raw: Record<string, unknown>) { const current = await getStoredTask(requiredText(raw.taskId, "任务标识")); if (!current) throw new TaskRepositoryError("TASK_NOT_FOUND", "任务不存在", 404); assertRevisionForExecute(current, raw); const next = mergeTaskUpdate(current, raw as TaskRequestBody); return this.write(operation, raw, () => saveStoredTask(parseTaskInput(next, current.id), { expectedUpdatedAt: requiredText(raw.expectedUpdatedAt, "expectedUpdatedAt") }), () => preview(current, next)); }
  private async taskSchedule(operation: string, raw: Record<string, unknown>) { const task = await getStoredTask(requiredText(raw.taskId, "任务标识")); if (!task) throw new TaskRepositoryError("TASK_NOT_FOUND", "任务不存在", 404); const input = parseTaskScheduleInput(raw); return this.write(operation, raw, () => scheduleStoredTask(task.id, input), () => this.schedulePreview(task, undefined, input)); }
  private async taskReschedule(operation: string, raw: Record<string, unknown>) { const task = await getStoredTask(requiredText(raw.taskId, "任务标识")); const event = await getStoredCalendarEvent(requiredText(raw.eventId, "日程标识")); if (!task) throw new TaskRepositoryError("TASK_NOT_FOUND", "任务不存在", 404); if (!event) throw new TaskRepositoryError("TIME_BLOCK_NOT_FOUND", "任务时间块不存在", 404); assertRevisionForExecute(event as { updatedAt: string }, raw); const input = parseTaskScheduleInput(raw); return this.write(operation, raw, () => updateStoredTaskSchedule(task.id, event.id, input, requiredText(raw.expectedUpdatedAt, "expectedUpdatedAt")), () => this.schedulePreview(event, event.id, input)); }
  private async taskCancel(operation: string, raw: Record<string, unknown>) { const event = await getStoredCalendarEvent(requiredText(raw.eventId, "日程标识")); if (!event) throw new TaskRepositoryError("TIME_BLOCK_NOT_FOUND", "任务时间块不存在", 404); assertRevisionForExecute(event as { updatedAt: string }, raw); return this.write(operation, raw, () => deleteStoredTaskSchedule(requiredText(raw.taskId, "任务标识"), event.id, requiredText(raw.expectedUpdatedAt, "expectedUpdatedAt")), () => preview(event, null)); }
  private async planCreate(operation: string, raw: Record<string, unknown>) { const input = planInput(raw); return this.write(operation, raw, () => saveStoredProjectPlanItem(input), () => preview(null, input)); }
  private async planUpdate(operation: string, raw: Record<string, unknown>) { const projectId = requiredText(raw.projectId, "项目标识"); const current = (await listStoredProjectPlanItems(projectId)).find((item) => item.id === requiredText(raw.planItemId, "计划项标识")); if (!current) throw new McpDomainInputError("计划项不存在"); assertRevisionForExecute(current, raw); const input = planInput({ ...current, ...raw, id: current.id, projectId, dependencyIds: raw.dependencyIds ?? current.dependencyIds }); return this.write(operation, raw, () => saveStoredProjectPlanItem(input, { expectedUpdatedAt: requiredText(raw.expectedUpdatedAt, "expectedUpdatedAt") }), () => preview(current, input)); }
  private async planDelete(operation: string, raw: Record<string, unknown>) { const projectId = requiredText(raw.projectId, "项目标识"); const current = (await listStoredProjectPlanItems(projectId)).find((item) => item.id === requiredText(raw.planItemId, "计划项标识")); if (!current) throw new McpDomainInputError("计划项不存在"); assertRevisionForExecute(current, raw); return this.write(operation, raw, async () => { if (!await deleteStoredProjectPlanItem(projectId, current.id, requiredText(raw.expectedUpdatedAt, "expectedUpdatedAt"))) throw new McpDomainInputError("计划项不存在"); return { deleted: true as const }; }, () => preview(current, null)); }
  private async planLink(operation: string, raw: Record<string, unknown>, unlink: boolean) { const task = await getStoredTask(requiredText(raw.taskId, "任务标识")); if (!task) throw new TaskRepositoryError("TASK_NOT_FOUND", "任务不存在", 404); assertRevisionForExecute(task, raw); if (!task.projectId) throw new McpDomainInputError("任务必须属于项目"); const projectId = requiredText(raw.projectId ?? task.projectId, "项目标识"); if (task.projectId !== projectId) throw new McpDomainInputError("任务不能迁移到其他项目"); const planItemId = unlink ? null : requiredText(raw.planItemId, "计划项标识"); if (!unlink) { const plan = (await listStoredProjectPlanItems(projectId)).find((item) => item.id === planItemId); if (!plan || plan.projectId !== projectId) throw new McpDomainInputError("计划项不属于任务项目"); } const next = mergeTaskUpdate(task, { projectId: task.projectId, planItemId }); return this.write(operation, raw, () => saveStoredTask(parseTaskInput(next, task.id), { expectedUpdatedAt: requiredText(raw.expectedUpdatedAt, "expectedUpdatedAt") }), () => preview(task, next)); }
  private async noteCreate(operation: string, raw: Record<string, unknown>) { const input = noteInput(raw); return this.write(operation, raw, async () => mapNote(await saveStoredNote(input)), () => preview(null, input)); }
  private async noteUpdate(operation: string, raw: Record<string, unknown>) { const current = await getStoredNote(requiredText(raw.noteId, "笔记标识")); if (!current) throw new McpDomainInputError("笔记不存在"); assertRevisionForExecute(current, raw); const input = noteInput({ ...current, ...raw, content: raw.content === undefined ? noteContentToMarkdown(current.content) : raw.content }); return this.write(operation, raw, async () => mapNote(await saveStoredNote({ ...input, id: current.id }, { expectedUpdatedAt: requiredText(raw.expectedUpdatedAt, "expectedUpdatedAt") })), () => preview(mapNote(current), input)); }
  private async noteAppend(operation: string, raw: Record<string, unknown>) { const current = await getStoredNote(requiredText(raw.noteId, "笔记标识")); if (!current) throw new McpDomainInputError("笔记不存在"); assertRevisionForExecute(current, raw); const input = noteInput({ ...current, content: [noteContentToMarkdown(current.content), normalizeMcpNoteContent(raw.content)].filter(Boolean).join("\n\n") }); return this.write(operation, raw, async () => mapNote(await saveStoredNote({ ...input, id: current.id }, { expectedUpdatedAt: requiredText(raw.expectedUpdatedAt, "expectedUpdatedAt") })), () => preview(mapNote(current), input)); }
  ...
  private async relationUnlink(operation: string, raw: Record<string, unknown>) { const id = requiredText(raw.linkId, "关联标识"); return this.write(operation, raw, async () => { if (!await deleteEntityLink(id)) throw new McpDomainInputError("关联不存在"); return { deleted: true as const }; }, () => preview({ linkId: id }, null)); }
  private async calendarEventCreate(operation: string, raw: Record<string, unknown>) { const input = calendarInput(raw); await assertOwnedCalendar(input.calendarId); return this.eventWrite(operation, raw, input, null); }
  private async calendarEventUpdate(operation: string, raw: Record<string, unknown>) { const current = await getStoredCalendarEvent(requiredText(raw.eventId, "日程标识")); if (!current) throw new McpDomainInputError("日程不存在"); if (current.calendarId !== requiredText(raw.calendarId, "日历标识")) throw new McpDomainInputError("日程不属于该日历"); assertRevisionForExecute(current as { updatedAt: string }, raw); const input = calendarInput({ ...current, ...raw, id: current.id }); await assertOwnedCalendar(input.calendarId); return this.eventWrite(operation, raw, input, current); }
  private async calendarEventDelete(operation: string, raw: Record<string, unknown>) { const current = await getStoredCalendarEvent(requiredText(raw.eventId, "日程标识")); if (!current) throw new McpDomainInputError("日程不存在"); if (current.calendarId !== requiredText(raw.calendarId, "日历标识")) throw new McpDomainInputError("日程不属于该日历"); assertRevisionForExecute(current as { updatedAt: string }, raw); return this.write(operation, raw, async () => { await deleteCalendarEvent(current.calendarId, current.id, undefined, requiredText(raw.expectedUpdatedAt, "expectedUpdatedAt")); return { deleted: true as const }; }, () => preview(current, null)); }
  private async eventWrite(operation: string, raw: Record<string, unknown>, input: UpsertCalendarEventInput, before: CalendarEvent | null) { const conflicts = () => listStoredCalendarEventConflicts({ calendarId: input.calendarId, start: input.start, end: input.end, excludeEventId: input.id }); return this.write(operation, raw, async () => { const found = await conflicts(); if (found.length && raw.allowConflicts !== true) throw scheduleConflict(found); return upsertCalendarEvent(input); }, async () => preview(before, input, await conflicts())); }
  private async schedulePreview(before: unknown, excludeEventId: string | undefined, input: { readonly calendarId: string; readonly start: string; readonly end: string }) { const conflicts = await listStoredCalendarEventConflicts({ ...input, excludeEventId }); return preview(before, input, conflicts); }
}
async function assertOwnedCalendar(calendarId: string): Promise<void> { if (!(await listStoredCalendars()).some((calendar) => calendar.id === calendarId)) throw new McpDomainInputError("日历不存在"); }
