...
async function main() {
  ...
  try {
    ...
    const predecessor = overview?.planItems.find((task) => task.id === predecessorPlan.id);
    let successor = overview?.planItems.find((task) => task.id === successorPlan.id);
    assert(overview?.planItems.length === 2, "only explicit plan items appear in the gantt chart");
    ...
    assert(
      overview!.planItems.find((task) => task.id === successorPlan.id)!.sortOrder
        < overview!.planItems.find((task) => task.id === predecessorPlan.id)!.sortOrder,
      "plan items can be reordered inside a project phase",
    );

    overview = await reorderStoredProjectMilestone({
      projectId: "project-test",
      kind: "milestone",
      itemId: milestone.id,
      phaseId: validationPhase.id,
    ...
    await reorderStoredProjectMilestone({
      projectId: "project-test",
      kind: "milestone",
      itemId: milestone.id,
      phaseId: phase.id,
    ...
    overview = await getStoredProjectOverview("project-test");
    successor = overview!.planItems.find((task) => task.id === successorPlan.id);
    ...
    assert(movedPlan.status === "done", "gantt plan can complete a plan item");
    ...
    assert(overview?.planItems.every((task) => !task.phaseId), "deleting a phase preserves and ungroups its plan items");
    assert(
    ...
    assert(overview?.planItems.every((task) => !task.dependencyIds.includes(predecessorPlan.id)), "deleting a plan item removes plan dependencies");
