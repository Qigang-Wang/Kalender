...
export async function PATCH(request: Request, context: CalendarAccountRouteContext) {
  ...
  if (typeof body?.mailEnabled === "boolean" && typeof body.calendarEnabled === "boolean") {
    ...
    if (featureEmail !== undefined && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(featureEmail)) {
      return NextResponse.json({ ok: false, message: "Bitte geben Sie eine gültige offizielle Postfachadresse ein" }, { status: 400 });
    }
    ...
    return account
      ...
      : NextResponse.json({ ok: false, message: "Devisenkonto existiert nicht" }, { status: 404 });
  }
  ...
  if (!/^#[0-9a-f]{6}$/.test(color)) {
    return NextResponse.json({ ok: false, message: "Bitte wählen Sie eine gültige Kalenderfarbe" }, { status: 400 });
  }
  const account = await updateCalendarAccountSettings(accountId, { displayName, color });
  if (!account) {
    return NextResponse.json({ ok: false, message: "Kalenderkonten existieren nicht" }, { status: 404 });
  }
