...
export async function GET(request: Request) {
  try {
    ...
    if (query.trim().length > 100) return NextResponse.json({ ok: false, message: "Suche nach nicht mehr als 100 Zeichen" }, { status: 400 });
    ...
    return NextResponse.json({ ok: false, message: "Suche ist vorerst nicht verfügbar" }, { status: 500 });
  }
}
