...
export async function PATCH(request: Request, context: RouteContext) {
  ...
  try {
    ...
    if (!current) throw new AiProviderError("Das KI-Modell existiert nicht", "AI_MODEL_NOT_FOUND", 404);
...
export async function DELETE(_request: Request, context: RouteContext) {
  const { modelId } = await context.params;
  try {
    if (!await deleteAiModel(modelId)) throw new AiProviderError("Das KI-Modell existiert nicht", "AI_MODEL_NOT_FOUND", 404);
